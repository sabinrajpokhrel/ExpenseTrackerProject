...
def export_csv():
    ...
    if export_response == "View":
        try:
            ...
            # Alternatively, you can specify other applications like Notepad, etc.
            # app_path = r"C:\Windows\System32\notepad.exe"
            # subprocess.Popen([app_path, output_file])
...
def open_link(element):  # Function to open any kind of external links(using webbrowser library)
    if element == "about_us":
        url = "E:/project_scratch/assets/ExpenseTrackerSoftcopySubmission.pdf"
        webbrowser.open(url)
...
def create_user():  # Function to create a new user(sign up)
    ...
    if existing_user:
        ...
        cursor.execute('''
            INSERT INTO users (username, password, first_name, last_name, coventry_id, gender)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (username, password, first_name, last_name, coventry_id, gender))
